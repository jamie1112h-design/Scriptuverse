// Scriptuverse -- Comfort Instrument (renamed from Refuge pre-launch;
// see the rename's own ScriptBDL decision once logged)
// Supabase Edge Function: comfort
// Project: l3v3l-scriptuverse (xvlqixdhxvsjcowjmxyl)
// Deploy to: Supabase dashboard -> Edge Functions -> New Function -> name: comfort
...
// Comfort-specific is OUTPUT_GALAXY_INSTRUCTIONS and the outer SYSTEM_PROMPT
// framing below.
//
...
// this must exist here too, not just on Counsel -- otherwise Comfort usage
...
// ── OUTPUT INSTRUCTIONS (Comfort-specific) ────────────────────────────────
...
const OUTPUT_GALAXY_INSTRUCTIONS = `You are generating Comfort's response -- presence and accompaniment for someone bringing grief, fear, or a felt distance from God. This is a different job from Counsel's dilemma-resolution, not a softer version of it.
...
// ── SYSTEM PROMPT ──────────────────────────────────────────────────────────
const SYSTEM_PROMPT = `You are Comfort, one of four instruments in Scriptuverse -- a denomination-aware, scripture-anchored advisory suite. Comfort offers presence and accompaniment to someone carrying grief, fear, or a felt distance from God -- not primarily a decision to work through, but a moment to be met in.
...
You will receive the person's fixed profile, all Layer 1 answers, and all Layer 2 answers. Produce Comfort's full response.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    //
    // FIX (found in testing-prep review, 2026-07-19): the crisis-fallback
    // chat (Decision 178) resends the ENTIRE growing message array on every
    // turn, and messages[0] is permanently the original seed turn -- so if
    // that seed was a [GENERATE_OUTPUT] call that got diverted into crisis
    // chat, the old check below matched on every single subsequent reply in
    // that conversation, silently decrementing trial_uses_remaining once per
    // crisis-chat turn instead of once per session. Requiring messages.length
    // === 1 restricts the match to the original one-shot call only -- true on
    // the very first submitL2() request (a lone user turn), never true again
    // once a second message has been appended, which is exactly when a
    // crisis-chat continuation begins.
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      console.error(`[scriptuverse-comfort] Empty reply on attempt ${attempt} of ${MAX_ATTEMPTS}.`);
    }
    ...
    if (crisisDetected && user) {
      ...
      // Given Decision 48's own note that Comfort's crisis-triage stakes
      ...
      // NOTE: logged instrument value switched from "refuge" to "comfort"
      // as part of the pre-launch rename -- flag if continuity with old
      // scriptuverse_crisis_events rows under "refuge" matters more than
      // consistency going forward.
      logCrisisEvent(user.id, "comfort").catch((e) =>
